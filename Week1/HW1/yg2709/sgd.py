...
def gradient_descent(input, model, loss, iterations=256):
    ...
    for _ in tqdm(range(iterations)):
        ...
        # add gaussian blur
        input.data = transforms.GaussianBlur(kernel_size=(3, 3), sigma=(0.1, 5.0))(input.data)

        # clamp the pixel values between 0 and 1
        input.data = input.data.clamp(min=0, max=1)
        ...
        # blur the gradients
        input.grad.data = transforms.GaussianBlur(kernel_size=(3, 3), sigma=(0.1, 5.0))(input.grad.data)

        # gradient clipping
        gradients = torch.clamp(input.grad, -0.5, 0.5)

        input.data += lr * gradients
